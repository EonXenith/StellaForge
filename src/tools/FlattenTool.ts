...
export class FlattenTool implements ITool {
  name = 'flatten';
  ...
  onStrokeStart(hit: BrushHit) {
    this.deltas.clear();
    this.targetHeight = this.planetData.heightmap[hit.vertexIndex];
    this.applyBrush(hit);
  }
  ...
  private applyBrush(hit: BrushHit) {
    ...
    for (const { index, weight } of vertices) {
      ...
      const t = brushStrength * weight * 5;
      this.planetData.heightmap[index] += (this.targetHeight - this.planetData.heightmap[index]) * Math.min(t, 1);
